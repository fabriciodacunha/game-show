...
import com.padrinho.gameshow.config.KafkaProperties;
...
class GameShowKafkaResourceIT {
    ...
    void setup() {
        ...
        GameShowKafkaResource kafkaResource = new GameShowKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/game-show-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/game-show-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
