...
/**
 * REST controller for managing {@link com.padrinho.gameshow.domain.Questao}.
 */
@RestController
@RequestMapping("/api")
public class QuestaoResource {
    ...
    public QuestaoResource(QuestaoService questaoService, QuestaoRepository questaoRepository) {
    ...
    public ResponseEntity<Questao> createQuestao(@RequestBody Questao questao) throws URISyntaxException {
        ...
        return ResponseEntity
            .created(new URI("/api/questaos/" + result.getId()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, true, ENTITY_NAME, result.getId().toString()))
            .body(result);
    }

    /**
     * {@code PUT  /questaos/:id} : Updates an existing questao.
     *
     * @param id the id of the questao to save.
     ...
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PutMapping("/questaos/{id}")
    public ResponseEntity<Questao> updateQuestao(@PathVariable(value = "id", required = false) final Long id, @RequestBody Questao questao)
        throws URISyntaxException {
        log.debug("REST request to update Questao : {}, {}", id, questao);
        if (questao.getId() == null) {
            throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
        }
        if (!Objects.equals(id, questao.getId())) {
    ...
    /**
     * {@code PATCH  /questaos/:id} : Partial updates given fields of an existing questao, field will ignore if it is null
     *
     * @param id the id of the questao to save.
     * @param questao the questao to update.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the updated questao,
     * or with status {@code 400 (Bad Request)} if the questao is not valid,
     * or with status {@code 404 (Not Found)} if the questao is not found,
     * or with status {@code 500 (Internal Server Error)} if the questao couldn't be updated.
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PatchMapping(value = "/questaos/{id}", consumes = { "application/json", "application/merge-patch+json" })
    public ResponseEntity<Questao> partialUpdateQuestao(
        @PathVariable(value = "id", required = false) final Long id,
        @RequestBody Questao questao
    ) throws URISyntaxException {
        log.debug("REST request to partial update Questao partially : {}, {}", id, questao);
        if (questao.getId() == null) {
            throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
        }
        if (!Objects.equals(id, questao.getId())) {
        ...
        Optional<Questao> result = questaoService.partialUpdate(questao);

        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, questao.getId().toString())
        );
    }
